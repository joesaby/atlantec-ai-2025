...
const EnhancedCharts = ({ resourceType }) => {
  ...
  const renderYearlyComparisonChart = () => {
    if (!yearlyComparisonRef.current || !userProgress.resourceUsage[resourceType]) return;
          ...
          tooltip: {
            callbacks: {
              label: function(context) {
                ...
                return `${context.dataset.label}: ${value.toFixed(1)} ${getResourceUnit(resourceType)}`;
              }
            }
          }
        },
  ...
  const renderSeasonalPatternChart = () => {
    if (!seasonalPatternRef.current || !userProgress.resourceUsage[resourceType]) return;
    ...
    const averages = seasons.map(season => 
      seasonCounts[season] > 0 ? 
        seasonData[season] / seasonCounts[season] : 0
    );
          ...
          tooltip: {
            callbacks: {
              label: function(context) {
                ...
                return `Average: ${value.toFixed(1)} ${getResourceUnit(resourceType)} per entry`;
              }
            }
          }
        }
  ...
  const renderHeatmapChart = () => {
    if (!heatmapRef.current || !userProgress.resourceUsage[resourceType]) return;
  ...
  const getResourceName = (type) => {
    switch(type) {
      ...
      case 'compost': return 'Compost Added';
      case 'harvest': return 'Food Harvested';
      case 'carbon': return 'Carbon Emissions';
      case 'energy': return 'Energy Usage';
      case 'waste': return 'Waste Reduced';
    ...
    <div className="bg-base-200 p-4 rounded-lg mb-6">
      ...
      <div className="bg-base-100 p-3 rounded-lg">
        ...
        <div style={{ height: "300px" }}>
          {chartView === 'yearly' && <canvas ref={yearlyComparisonRef}></canvas>}
          {chartView === 'seasonal' && <canvas ref={seasonalPatternRef}></canvas>}
      ...
      <div className="text-xs mt-2 text-center text-base-content/60">
        <p>Click on different tabs to explore various ways of visualizing your data</p>
      </div>
    </div>
  );
};
